...
function MovieDetails() {
  ...
  useEffect(() => {
    const fetchMovieDetails = async () => {
      ...
      try {
        const response = await axios.get(`https://movie-ticket-booking-6jpv.vercel.app//moviess/${id}`);
  ...
  const handletime = async (showtime, theater) => {
    ...
    try {
      ...
      const response = await axios.get(`https://movie-ticket-booking-6jpv.vercel.app//movies/${id}/${encodedTheater}/${showtime}/${encodedDate}`);
